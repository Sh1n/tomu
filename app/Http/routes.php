...
Route::get('/auth/redirect/{provider}', 'Auth\AuthController@redirectToProvider');
Route::get('/auth/{provider}/callback', 'Auth\AuthController@handleProviderCallback');
Route::get('/auth/user',                'Auth\AuthController@user');

Route::get('/imagesharer/{pathName}/{operaIndex}', 'ToMuController@imageSharer');

Route::get('/qrcodes', function(){
    $percorsi = \App\PercorsiService::getPercorsi();
   return view('qr_codes', ['percorsi' => $percorsi]);
});

Route::get('/qr', function(){
    $url = \Illuminate\Support\Facades\URL::to(env('APP_URL'));
    ...
    $qrCode
        ...
        ->setForegroundColor(['r' => 115, 'g' => 39, 'b' => 132, 'a' => 0])
        ->setBackgroundColor(['r' => 255, 'g' => 255, 'b' => 255, 'a' => 0])
        ->setLabelFontSize(16)
//        ->setLogo("images/me.jpg")
        ->setImageType(\Endroid\QrCode\QrCode::IMAGE_TYPE_PNG)
    ;

// now we can directly output the qrcode

    $response = new \Illuminate\Http\Response($qrCode->get(), 200, ['Content-Type' => $qrCode->getContentType()]);
    return $response;
});


Route::get('/generateQrCode/{pathName}/{operaIndex}', function($pathName, $operaIndex){
